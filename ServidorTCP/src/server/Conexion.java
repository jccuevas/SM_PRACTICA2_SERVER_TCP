				...
				while ((inputData = inputStream.readLine()) != null && !salir) {
					...
					if (fields.length == 1) {// No hay espacios, debe ser solo
						...
						parametro = null;
													...
													// ya qe el método split ha
													// devuelto dos o más
													// cadenas
						comando = fields[0];// El comando debe ser el primer
											// campo
						// Se extrae el parámetro que se considera que es todo
					...
					case S_USER:// Estado USER
						if (comando.equalsIgnoreCase(QUIT)) {// En todos los
							...
							tempUser = parametro;
							outputData = OK + CRLF;
							estado++;// Se incrementa el estado aunque aún no se
										// compara
						} else {// No es un comando válido
						...
						if (comando.equalsIgnoreCase(QUIT)) {
							...
							if (parametro != null)
								outputData = OK + SP + parametro + CRLF;// se
																		...
																		// cadena
																		// recibida
							else// El cliente no envió parámetro alguno
								outputData = ERROR + CRLF;
						} else// No es un comando válido
							outputData = ERROR + CRLF;
						break;
